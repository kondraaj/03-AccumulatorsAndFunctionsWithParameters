...
def run_test_even_better_draw_circles():
    ...
    print('--------------------------------------------------')
    even_better_draw_circles(7, 24, 12, 13, 0.1)


def even_better_draw_circles(radius_variance, number_of_circles, center_x, center_y, render_time):
    window = rg.RoseWindow(400, 400)

    for k in range(number_of_circles):
        center = rg.Point(k * center_x, k * center_y)
        circle = rg.Circle(center, radius_variance * k)
        circle.attach_to(window)
        window.render(render_time)
